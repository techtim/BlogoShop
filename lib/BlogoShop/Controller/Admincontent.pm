...
use constant BANNER_PARAMS => qw(id link image category weight type name descr);
...
#            warn "DELETE $cat \-\> $par->{$key}";
	...
	foreach my $cat (@cats) {
		if ($self->req->param('pos.cat.'.$cat->{_id})) {
	...
	$self->app->db->stuff->remove({_id => 'active_categories'}) if $self->req->method eq 'POST'; #remove to update active cats from Utils
