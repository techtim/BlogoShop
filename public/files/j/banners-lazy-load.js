...
function checkBannerPosition(options){
...
(function($,global) {
	...
	var logError = isFunction(global.console) ? 
			doLog : ignore;
	...
	function sanitize(html,options,parentQ,parentContext) {
		...
		html = html.replace('LANGUAGE=VBScript','type="text/javascript"')
		html = html.replace('on error resume next','')
		//html = html.replace('plugin=(IsObject(CreateObject("ShockwaveFlash.ShockwaveFlash.6")))','plugin=new ActiveXObject("ShockwaveFlash.ShockwaveFlash.6"); plugin = (typeof plugin == "object") ? true : false;')
		html = html.replace('plugin=(IsObject(CreateObject("ShockwaveFlash.ShockwaveFlash.6")))','plugin=true')
